...
const downloadCss = `@media(max-width:900px){.download-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:680px){.download-grid{grid-template-columns:1fr}}`;
...
const page = (title, content, scripts = "", origin = "") => {
  ...
  return `<!doctype html><html lang="fa" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#123f38"><meta name="description" content="کتابخانه خواندن و شنیدن فارسی با ترجمه جمله‌ای و تمرین شنیداری">${social}<title>${escape(title)}</title><style>${css}${handbookCss}${downloadCss}</style></head><body>${globalControls}${content}<script>${commonScript}</script><script type="module">${piperClientScript}</script>${scripts}</body></html>`;
};
