...
async function ensurePiper() {
  ...
  if (piperUnavailable) throw new Error('Piper unavailable');
  try {
    piperModule = await import('https://esm.sh/@mintplex-labs/piper-tts-web@1.0.5?bundle&deps=onnxruntime-web@1.18.0');
    return piperModule;
  } catch (error) {
...
function progressMessage(progress) {
  if (!progress || String(progress.url || '').startsWith('tts://')) return 'در حال ساخت صدا…';
  if (progress.total > 0) return 'در حال آماده‌سازی صدای فارسی: ' + Math.round(progress.loaded * 100 / progress.total) + '٪';
  return 'در حال آماده‌سازی صدای فارسی…';
}

function speakWithBrowser(row, token) {
  ...
  setStatus('صدای جایگزین مرورگر در حال پخش است.');
...
async function readRow(row) {
  ...
  setStatus('در حال آماده‌سازی صدای Piper…');
  try {
    ...
    const wav = await piper.predict({ text: row.dataset.text || '', voiceId: 'fa_IR-amir-medium' }, function (progress) {
      if (token === requestToken) setStatus(progressMessage(progress));
    });
...
const page = (title, content, scripts = "", origin = "") => {
  ...
  return `<!doctype html><html lang="fa" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#123f38"><meta name="description" content="کتابخانه خواندن و شنیدن فارسی با ترجمه جمله‌ای و تمرین شنیداری">${social}<title>${escape(title)}</title><style>${css}</style></head><body>${globalControls}${content}<script>${commonScript}</script>${scripts}</body></html>`;
};
...
function home(origin) {
  const categoryOrder = ["علوم و فناوری", "شاهنامه", "شعر", "نثر"];
