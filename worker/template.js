...
const HANDBOOKS = {
  ...
  studentPdf: "AFH1_2025_Farsi_Student_Study_Handbook.pdf",
  teacherPdf: "AFH1_2025_Farsi_English_Teacher_Handbook.pdf",
};
...
@media(max-width:900px){.download-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:680px){.download-grid{grid-template-columns:1fr}}
.download-hero-actions{display:flex;flex-wrap:wrap;gap:9px;margin-top:24px}.download-groups{margin-top:48px}.download-group{margin-bottom:42px}.download-group h2{margin:0 0 14px;padding-bottom:10px;border-bottom:1px solid var(--line);color:var(--green);font-family:var(--font-reading);font-size:1.65rem}.download-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.download-card{padding:19px;display:flex;flex-direction:column;gap:14px;background:var(--surface);border:1px solid var(--line);border-radius:18px}.download-card h3{margin:0;color:var(--green);font-family:var(--font-reading);font-size:1.2rem}.download-card p{margin:0;color:var(--muted);font-size:.76rem}.download-card-actions{display:flex;flex-wrap:wrap;gap:7px;margin-top:auto}.download-card-actions a{padding:7px 10px;border:1px solid var(--line);border-radius:9px;color:var(--green);font-size:.75rem;font-weight:800}.download-card-actions a:hover{background:var(--green);border-color:var(--green);color:white}
...
const downloadCss = `.week-navigation input{width:5.5rem;padding:7px 9px;border:1px solid var(--line);border-radius:9px;background:var(--surface);color:var(--ink);font:inherit}@media(max-width:900px){.download-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:680px){.download-grid{grid-template-columns:1fr}}`;
...
function reportProgress(event) {
  ...
  activeRequest.client.postMessage({
    type: 'progress',
    id: activeRequest.id,
...
async function synthesize(message, client) {
  ...
  activeRequest = { id: id, client: client };
  try {
    ...
    client.postMessage({ type: 'result', id: id, blob: blob });
  } finally {
    activeRequest = null;
  }
}

function enqueue(message, client) {
  if (message?.type !== 'synthesize') return;
  workQueue = workQueue.then(async function () {
    try { await synthesize(message, client); }
    catch (error) {
      client.postMessage({ type: 'error', id: Number(message?.id) || 0, message: String(error?.message || 'Piper synthesis failed') });
    }
  });
}
...
self.addEventListener('connect', function (event) {
  ...
  port.addEventListener('message', function (messageEvent) { enqueue(messageEvent.data, port); });
  port.start();
});

self.addEventListener('message', function (event) {
  enqueue(event.data, self);
});
`;

const piperClientScript = String.raw`
const piperApi = window.farsiPiper = {
  state: 'loading',
  message: 'Piper در حال آماده‌سازی است؛ صدای مرورگر فعلاً فعال است.',
  requestId: 0,
  requests: new Map(),
  cache: new Map(),
  transport: null,
...
function announcePiper(state, message) {
  piperApi.state = state;
  piperApi.message = message;
  const status = document.querySelector('[data-voice-status]');
  if (status) status.textContent = message;
  window.dispatchEvent(new CustomEvent('farsi-piper-state', { detail: { state: state, message: message } }));
}

function connectPiper() {
  if (piperApi.transport) return piperApi.transport;
  if (!window.WebAssembly || !navigator.storage?.getDirectory) throw new Error('Piper storage is unavailable');
  if (window.SharedWorker) {
    try {
      const sharedWorker = new SharedWorker('/piper-worker.js?v=3', { type: 'module', name: 'farsi-reader-piper' });
      piperApi.transport = sharedWorker.port;
      piperApi.shared = true;
      sharedWorker.port.start();
    } catch {}
  }
  if (!piperApi.transport && window.Worker) {
    piperApi.transport = new Worker('/piper-worker.js?v=3', { type: 'module', name: 'farsi-reader-piper' });
  }
  if (!piperApi.transport) {
    throw new Error('Web workers are unavailable');
  }
  piperApi.transport.addEventListener('message', function (event) {
    const message = event.data || {};
    const request = piperApi.requests.get(Number(message.id));
    if (!request) return;
    if (message.type === 'progress') {
      const loaded = Math.max(0, Number(message.loaded) || 0);
      const total = Math.max(0, Number(message.total) || 0);
      if (total > 0) announcePiper('loading', 'در حال دریافت صدای Piper: ' + Math.min(100, Math.round(loaded * 100 / total)) + '٪');
      else if (loaded > 0) announcePiper('loading', 'در حال دریافت صدای Piper: ' + (loaded / 1048576).toFixed(1) + ' مگابایت');
      return;
    }
    clearTimeout(request.timer);
    piperApi.requests.delete(Number(message.id));
  ...
  piperApi.transport.addEventListener('error', function () {
    announcePiper('error', 'Piper آماده نشد؛ صدای مرورگر فعال است.');
  });
  return piperApi.transport;
}

piperApi.request = function (text, firstRequest) {
  const clean = String(text || '').trim().slice(0, 500);
  if (piperApi.cache.has(clean)) return Promise.resolve(piperApi.cache.get(clean));
  let transport;
  try { transport = connectPiper(); }
  catch (error) { return Promise.reject(error); }
  const id = piperApi.requestId + 1;
  piperApi.requestId = id;
  return new Promise(function (resolve, reject) {
    const timer = setTimeout(function () {
      if (!piperApi.requests.has(id)) return;
      piperApi.requests.delete(id);
      reject(new Error('Piper timed out'));
    }, firstRequest ? 180000 : 45000);
    piperApi.requests.set(id, {
      timer: timer,
      resolve: function (blob) {
        piperApi.cache.set(clean, blob);
        if (piperApi.cache.size > 8) piperApi.cache.delete(piperApi.cache.keys().next().value);
        resolve(blob);
      },
      reject: reject
    });
    transport.postMessage({ type: 'synthesize', id: id, voiceId: PIPER_VOICE_ID, text: clean });
  });
};

announcePiper('loading', 'Piper به‌طور خودکار در حال آماده‌سازی است؛ بار نخست نزدیک به ۹۰ مگابایت دریافت می‌شود.');
piperApi.ready = piperApi.request('سلام', true).then(function (blob) {
  announcePiper('ready', 'صدای Piper آماده است.');
  return blob;
}).catch(function (error) {
  announcePiper('error', 'Piper آماده نشد؛ صدای مرورگر فعال است.');
  throw error;
});
piperApi.ready.catch(function () {});
`;

const readerScript = String.raw`
document.querySelectorAll('.sentence-text').forEach(function (node) {
  const fragment = document.createDocumentFragment();
  String(node.textContent || '').split(/(\s+)/u).forEach(function (part) {
    if (!part) return;
    if (/^\s+$/u.test(part)) fragment.appendChild(document.createTextNode(part));
    else {
...
async function readRow(row) {
  ...
  try {
    const wav = await window.farsiPiper.request(row.dataset.text || '', false);
...
function syncPiperState(event) {
  const state = event?.detail?.state || window.farsiPiper?.state || 'error';
  const message = event?.detail?.message || window.farsiPiper?.message || '';
  voiceMode = state === 'ready' ? 'piper' : 'browser';
  if (voiceBadge) voiceBadge.textContent = state === 'ready' ? 'Piper · فارسی' : state === 'loading' ? 'Piper در حال آماده‌سازی' : 'صدای مرورگر';
...
window.addEventListener('farsi-piper-state', syncPiperState);
syncPiperState();
...
const page = (title, content, scripts = "", origin = "") => {
  ...
  const staticAssets = origin.endsWith("/farsi-reader-library");
  ...
  const styles = staticAssets ? `<link rel="stylesheet" href="${origin}/assets/site.css">` : `<style>${css}${handbookCss}${downloadCss}</style>`;
  const runtime = staticAssets
    ? `<script src="${origin}/assets/common.js"></script><script type="module" src="${origin}/assets/piper-client.js"></script>${scripts ? `<script type="module" src="${origin}/assets/reader.js"></script>` : ""}`
    : `<script>${commonScript}</script><script type="module">${piperClientScript}</script>${scripts}`;
  return `<!doctype html><html lang="fa" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#123f38"><meta name="description" content="کتابخانه خواندن و شنیدن فارسی با ترجمه جمله‌ای و تمرین شنیداری">${social}<title>${escape(title)}</title>${styles}</head><body>${globalControls}${content}${runtime}</body></html>`;
};

function siteHeader(active = "library") {
  return `<header class="site-header"><a class="wordmark" href="/"><span class="wordmark-mark">ف</span><span>خوانش فارسی</span></a><nav aria-label="راهبری اصلی"><a class="${active === "library" ? "active" : ""}" href="/">کتابخانه</a><a href="/#${encodeURIComponent("شاهنامه")}">شاهنامه</a><a class="${active === "downloads" ? "active" : ""}" href="/library-downloads">دریافت‌ها</a><a class="${active === "resources" ? "active" : ""}" href="/resources">منابع پیشرفته</a></nav></header>`;
}

function siteFooter() {
  return `<footer class="site-footer"><span>خواندن، شنیدن و گفت‌وگو به فارسی.</span><span><a href="/library-downloads">همهٔ فایل‌ها</a> · <a href="/resources">منابع تکمیلی</a></span></footer>`;
}

function completeDownloadLinks(reader, labelPrefix = "") {
  if (!reader.student || !reader.teacher) return "";
  const prefix = labelPrefix ? `${labelPrefix} ` : "";
  const word = `<a href="/downloads/${reader.student}">${prefix}دانشجو (Word)</a><a href="/downloads/${reader.teacher}">${prefix}مدرس (Word)</a>`;
  if (reader.slug !== "afh1-2025") return word;
  return `${word}<a href="/downloads/${HANDBOOKS.studentPdf}">${prefix}دانشجو (PDF)</a><a href="/downloads/${HANDBOOKS.teacherPdf}">${prefix}مدرس (PDF)</a>`;
}
...
function readerPage(reader, week, teacher, origin) {
  ...
  const picker = `<label>بخش <input type="number" inputmode="numeric" min="1" max="${reader.weeks.length}" value="${week.number}" aria-label="انتخاب بخش از ${faNumber(reader.weeks.length)} بخش" onchange="window.location.href='/read/${reader.slug}/'+this.value+'${teacher ? "?edition=teacher" : ""}'"> از ${faNumber(reader.weeks.length)}</label>`;
  const downloads = reader.student && reader.teacher ? `<section class="reader-downloads"><p>دریافت نسخهٔ کامل</p>${completeDownloadLinks(reader)}</section>` : "";
  const tools = `<section class="reader-tools" aria-label="ابزارهای خواندن"><button class="tool-button" type="button" data-listening-toggle aria-pressed="false">حالت شنیداری</button><button class="tool-button" type="button" data-play-toggle aria-pressed="false">پخش</button><button class="tool-button" type="button" data-next-sentence>جمله بعد</button><span class="voice-badge" data-voice-badge>Piper در حال آماده‌سازی</span><span class="voice-status" data-voice-status aria-live="polite">Piper به‌طور خودکار در حال آماده‌سازی است.</span></section>`;
...
function download(name) {
  ...
  return new Response(bytes, { headers: { "content-type": contentType, "content-disposition": `attachment; filename="${name}"` } });
}
...
export const staticSite = {
  piperWorkerScript,
  assets: {
    "site.css": `${css}${handbookCss}${downloadCss}`,
    "common.js": commonScript,
    "piper-client.js": piperClientScript,
    "reader.js": readerScript,
  },
  render(pathname, teacher = false, origin = "") {
    const route = pathname.split("/").filter(Boolean).map(decodeURIComponent);
    if (route.length === 0) return home(origin);
    if (route[0] === "resources" && route.length === 1) return resourcesPage(origin);
    if (route[0] === "library-downloads" && route.length === 1) return downloadsPage(origin);
    if (route[0] === "read" && route.length === 3) {
      const reader = READERS.find((item) => item.slug === route[1]);
      const week = reader?.weeks.find((item) => item.number === Number(route[2]));
      if (!reader || !week) throw new Error(`Reader route not found: ${pathname}`);
      return readerPage(reader, week, teacher, origin);
    }
    throw new Error(`Static route not found: ${pathname}`);
  },
};
...
export default {
  async fetch(request) {
    ...
    if (path[0] === "library-downloads" && path.length === 1) return new Response(downloadsPage(url.origin), { headers: { "content-type": "text/html; charset=utf-8" } });
